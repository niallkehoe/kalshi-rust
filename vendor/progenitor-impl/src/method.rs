...
impl Generator {
    ...
    pub(crate) fn extract_responses<'a>(
        ...
        // Patched: instead of asserting, log the conflict and use the first
        // typed response (prefer Type over None/Raw).
        if response_types.len() > 1 {
            eprintln!(
                "progenitor-impl patch: operation has {} distinct success response types: {:?}; \
                 keeping the first. response_items statuses: {:?}",
                response_types.len(),
                response_types
                    .iter()
                    .map(|t| format!("{t:?}"))
                ...
                response_items
                    .iter()
                    .map(|r| format!("{:?}={:?}", r.status_code, r.typ))
                    .collect::<Vec<_>>(),
            );
        }
        let response_type = response_types
            .into_iter()
            // Prefer Type > Raw > None > Upgrade ordering
            .max_by_key(|t| match t {
                OperationResponseKind::Type(_) => 3,
                OperationResponseKind::Raw => 2,
                OperationResponseKind::None => 1,
                OperationResponseKind::Upgrade => 0,
            })
